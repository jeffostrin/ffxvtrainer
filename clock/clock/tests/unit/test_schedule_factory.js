...
function assertHourlySchedule(actual, expected) {
  ...
  expect(actual.isCurrentHepoch, "for " + actual.hepoch).to.equal(expected.isCurrentHepoch);
  expect(actual.hepochReadable, "for " + actual.hepoch).to.equal(expected.hepochReadable);
  expect(actual.secondsUntil, "for " + actual.hepoch).to.equal(expected.secondsUntil);
  expect(actual.timeUntil, "for " + actual.hepoch).to.equal(expected.timeUntil);
  expect(actual.events.mini, "for " + actual.hepoch).to.equal(expected.events.mini);
  expect(actual.events.luna, "for " + actual.hepoch).to.equal(expected.events.luna);
  expect(actual.events.fourHour, "for " + actual.hepoch).to.equal(expected.events.fourHour);
  expect(actual.events.other, "for " + actual.hepoch).to.equal(expected.events.other);
}
...
describe('Tests lookup', function () {
  ...
  it('create simple schedule', async () => {
    ...
    var schedule = new ScheduleFactory().at(ctime).forHepochs(30, 35).forRotations(eventRotations).create();

console.log("result:");
console.log(schedule);
    expect(schedule.length).to.equal(6)
    assertHourlySchedule(schedule[0], { hepoch:30, isCurrentHepoch: true, hepochReadable: "10:00pm (01-01)", secondsUntil: 0, timeUntil: "now", events: { luna: "Luna Gifts", fourHour: "Research (cont)"} });
    assertHourlySchedule(schedule[1], { hepoch:31, isCurrentHepoch: false, hepochReadable: "11:00pm (01-01)", secondsUntil: 1 * SECONDS_IN_HOUR, timeUntil: "in 1:00", events: { luna: "", fourHour: "Empire Ascend" } });
    assertHourlySchedule(schedule[2], { hepoch:32, isCurrentHepoch: false, hepochReadable: "12:00am (01-02)", secondsUntil: 2 * SECONDS_IN_HOUR, timeUntil: "in 2:00", events: { luna: "", fourHour: "Empire Ascend (cont)" } });
